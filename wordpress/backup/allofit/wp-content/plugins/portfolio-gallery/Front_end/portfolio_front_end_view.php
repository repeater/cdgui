...
<?php } ?>
         <div id="huge_it_portfolio_container_<?php echo $portfolioID; ?>" class="super-list variable-sizes clearfix" <?php if($paramssld["ht_view0_sorting_float"] == "top" && $paramssld["ht_view0_filtering_float"] == "top") echo "style='clear: both;'";?>>
              ...
              foreach($images as $key=>$row)
              {
                      ...
                      ?>
                      <div class="portelement_<?php echo $portfolioID; ?> pcolorbox_grouping <?php foreach ($catForFilter as $catForFilterValue) { echo str_replace(" ","-",$catForFilterValue)." ";} ?>" data-symbol="<?php echo $row->name; ?>" data-category="alkaline-earth">
  ...
  jQuery(document).ready(function(){
	...
	jQuery('.portelement_<?php echo $portfolioID; ?> .image-block_<?php echo $portfolioID; ?> .image-overlay a').on('click',function(){//console.log("filter");//
        ...
        if(jQuery('.pupup-element.active .description').height() + jQuery('.right-block h3').height() + 200 > jQuery('.pupup-element.active .right-block').height()){
            if(jQuery('.pupup-element.active img').height() > jQuery('.pupup-element.active .image-block_<?php echo $portfolioID; ?>').height()){
                jQuery('.pupup-element.active .right-block').css('overflow-y', '');
                jQuery('.pupup-element.active .popup-wrapper_<?php echo $portfolioID; ?>').css('overflow-y', 'auto');
            }else{
	...
	jQuery('#huge_it_portfolio_popup_list_<?php echo $portfolioID; ?> .popup-wrapper_<?php echo $portfolioID; ?> .right-block ul.thumbs-list_<?php echo $portfolioID; ?> li ').on('click','a.img-thumb',function(){//console.log("filter");//thubnail neri meccnelna
        ...
        jQuery("body").keydown(function(e) {
                if(e.keyCode == 37) {
                    jQuery("#huge_it_portfolio_popup_list_<?php echo $portfolioID; ?> .pupup-element.active .heading-navigation_<?php echo $portfolioID; ?> .left-change").trigger("click");
                }
            }); 
...
//            alert(num+" "+cnt);
      ...
      jQuery("body").keydown(function(e) {
                if(e.keyCode == 39) {
                    jQuery("#huge_it_portfolio_popup_list_<?php echo $portfolioID; ?> .pupup-element.active .heading-navigation_<?php echo $portfolioID; ?> .right-change").click();
                }
            });
        ...
        jQuery("body").keydown(function(e) {
            if(e.keyCode == 27) {
                jQuery('#huge_it_portfolio_popup_list_<?php echo $portfolioID; ?> .heading-navigation_<?php echo $portfolioID; ?> .close').click();
            }
        });
...
#huge_it_portfolio_content_<?php echo $portfolioID; ?> #huge_it_portfolio_options_<?php echo $portfolioID; ?> {
    ...
    echo "display:none;";
    if($paramssld["ht_view2_filtering_float"] == 'left' && $paramssld["ht_view2_sorting_float"] == 'none') { if($portfolioShowFiltering == "on") { echo "margin-left: 0%;"; } else { echo "margin-left: 0%;"; } }
					...
					<div><ul class="thumbs-list_<?php echo $portfolioID; ?>">
						...
						array_shift($imgurl);
								...
								<?php 
										...
										$videourl=get_video_id_from_url_portfolio($img);?>
										<li>
										<a href="https://www.youtube.com/embed/<?php echo $videourl[0];?>" class="huge_it_portfolio_item pyoutube portfolio-group-slider_<?php echo $portfolioID;?>_<?php  echo $group_key1; ?>  add-H-relative"  title = "<?php echo $row->name; ?>">
											<img src="//img.youtube.com/vi/<?php echo $videourl[0]; ?>/mqdefault.jpg"><div class="play-icon <?php echo $videourl[1];?>-icon"></div>
										</a>
										</li>	
									<?php
